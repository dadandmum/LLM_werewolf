...
def draw_radar(input):
    ...
    value_labels=[x for x in value_labels if x != "" ]
    ...
    for t in range(4):
        ...
        ax.set_xticklabels(value_labels,fontsize=12, fontname='Arial')
        
        ax.set_yticks(np.arange(np.floor(y_min*10)*0.1,np   .ceil(y_max*10)*0.1,0.1))
        ax.set_ylim(np.floor(y_min*10)*0.1,np.ceil(y_max*10)*0.1)
        
        ax.set_title(set_labels[t+1], y=1.08 , fontsize=22,fontname='Times New Roman')
